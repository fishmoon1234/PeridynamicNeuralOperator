    ...
    for index in range(1):
        ...
        ################################################################
            ...
            if if_compute_error == 1:
                ...
                for batch in test_loader:
                    ...
                    if j == 0:
                        ...
                        plot_u(batch.u.cpu().detach().numpy().reshape(S, S, 2), base_dir, ex, j)
            ...
            if plot_model == 1:
                ...
                xi_norm = torch.linspace(0.05,0.25, 100).reshape(-1,1)
                ...
                plt.legend()
                plt.xlabel(r'$|\boldsymbol{\xi}|$', fontsize=fontsize)
                plt.ylabel(r'$k(|\boldsymbol{\xi}|)$', fontsize=fontsize)
                plt.grid(True)
                for spine in ax.spines.values():
                    spine.set_visible(True)
                plt.savefig('%s/2D_%s_k.png' % (base_dir, ex), format='png', bbox_inches='tight',dpi=300)
                plt.close()
