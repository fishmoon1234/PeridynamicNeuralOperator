...
def plot_u(u_plot, base_dir, ex, n):
    ...
    vmin_u1 = u_plot[:,:,0].min()
    vmax_u1 = u_plot[:,:,0].max()
    vmin_u2 = u_plot[:,:,1].min()
    vmax_u2 = u_plot[:,:,1].max()
    
    # Plot u_1
    im0 = axes[0].imshow(u_plot[:,:,0], vmin=vmin_u1, vmax=vmax_u1, aspect='equal')
    axes[0].set_title('$u_1$', fontweight='bold', pad=15)
    axes[0].set_xlabel('$x_1$')
    axes[0].set_ylabel('$x_2$')
    cbar0 = fig.colorbar(im0, ax=axes[0], shrink=0.9, aspect=20)
    
    # Plot u_2
    im1 = axes[1].imshow(u_plot[:,:,1], vmin=vmin_u2, vmax=vmax_u2, aspect='equal')
    axes[1].set_title('$u_2$', fontweight='bold', pad=15)
    axes[1].set_xlabel('$x_1$')
    axes[1].set_ylabel('$x_2$')
    cbar1 = fig.colorbar(im1, ax=axes[1], shrink=0.9, aspect=20)
    
    plt.tight_layout()
    plt.show()
    plt.savefig('%s/2D_%s_u%s.png' % (base_dir, ex, n), format='png', dpi=300, bbox_inches='tight')
    plt.close()
