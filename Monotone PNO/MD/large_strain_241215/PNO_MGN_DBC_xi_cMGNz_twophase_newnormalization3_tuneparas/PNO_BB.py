...
def main():
        ...
        for learning_rate in lrs:
            for scheduler_gamma in gammas:
                for weight_decay in wds:
                    print(f'>> ntrain: {ntrain}, lr: {learning_rate}, gamma: {scheduler_gamma}, w_d: {weight_decay}')
                    base_dir = '%s/Results/%s_nvalid%s_%s_lr%s_dr%s_step%s_bs%s_norm%s_z_beta%s' % (current_dir,INDEX_NAME, nvalid, layer_info, learning_rate , scheduler_gamma, scheduler_step, batch_size, norm, args.beta)
    ...
    ################################################################
        ...
        results_name = '%s_nvalid50_%s_lr%s_dr0.5_step100_bs10_norm1_z_beta50.0' % (INDEX_NAME, layer_info, args.lrs)
        ...
        def test(data, name, ntest):
            ...
            for batch in test_loader:
                ...
                if args.solution_type =='one_phase':
                    ...
                    print(f"{j}, err_b:{err_b_test[j]:.2e}, err_u:{err_u_test[j]:.2e}, fsolve flag:{ier}, {np.max(np.abs(info['fvec']))}, {info['nfev']}\n")
                else:
                    ...
                    print('linear:', f"{j}, err_b:{err_b_test[j]:.2e}, err_u:{err_u_test_linear[j]:.2e}, fsolve flag:{ier}, {np.max(np.abs(info['fvec']))}, {info['nfev']}")
                    
                    u, info, ier, msg = fsolve(func, u_linear, xtol=1e-9, full_output=True)
                    u = torch.tensor(u, dtype=torch.float32)
                    err_u_test[j] = torch.norm(u-u_true_i.view(-1,))/torch.norm(u_true_i.view(-1,))
                    print('nonlinear:', f"{j}, err_b:{err_b_test[j]:.2e}, err_u:{err_u_test[j]:.2e}, fsolve flag:{ier}, {np.max(np.abs(info['fvec']))}, {info['nfev']}")
        ...
        if plot_g == 1:
            ...
            ax.plot(lambdaa, k1_NN, color='deepskyblue', linewidth=3.0, linestyle='--',  markeredgecolor='deepskyblue')
            ...
            ax.set_title(r'$g(\lambda)$', fontweight='bold')
        ...
        if plot_k == 1:
            ...
            plt.title(r'$k(\boldsymbol{\xi})$', fontweight='bold')
            plt.xlabel(r'$\xi_1$')
            plt.ylabel(r'$\xi_2$')
            ax.grid(True)
            plt.tight_layout()
