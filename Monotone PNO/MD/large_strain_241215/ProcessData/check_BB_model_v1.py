...
def main():
    ...
    B_norm_train = np.sqrt(torch.mean((torch.norm(f_train, dim=-1))**2, dim=[1,2]))
    B_norm_valid = np.sqrt(torch.mean((torch.norm(f_valid, dim=-1))**2, dim=[1,2]))
    B_norm_test = np.sqrt(torch.mean((torch.norm(f_test, dim=-1))**2, dim=[1,2]))
    ...
    if if_compute_lambda == 1:
        ...
        err = np.zeros((nvalid, 1)) 
        # B_norm = np.zeros((ntrain, 1)) 
        lambda_min_max = np.zeros((nvalid, 2))     
        for i in range(nvalid):
            col, row = edge_index
            ksi = x_train[col] - x_train[row]
            eta = u_valid[i,col] - u_valid[i,row]
            ksi_norm = torch.norm(ksi, dim=1).unsqueeze(1)
            ksi_plus_eta = ksi+eta
            ksi_plus_eta_norm = torch.norm(ksi_plus_eta, dim=1).unsqueeze(1)
            extension = ksi_plus_eta_norm - ksi_norm
            ...
            abs_err = np.sqrt(torch.mean(torch.norm(L+f_valid[i,:,:,:], dim=-1)**2))
            # # abs_err = np.sqrt(torch.mean(torch.norm(L, dim=-1)**2))
            # B_norm[i] = np.sqrt(torch.mean(torch.norm(data_f[i,:,:,:], dim=-1)**2))
            err[i] = abs_err/B_norm_valid[i]
        ...
        err = np.zeros((ntest, 1)) 
        # B_norm = np.zeros((ntrain, 1)) 
        lambda_min_max = np.zeros((ntest, 2))     
        for i in range(ntest):
            col, row = edge_index
            ksi = x_train[col] - x_train[row]
            eta = u_test[i,col] - u_test[i,row]
            ksi_norm = torch.norm(ksi, dim=1).unsqueeze(1)
            ksi_plus_eta = ksi+eta
            ksi_plus_eta_norm = torch.norm(ksi_plus_eta, dim=1).unsqueeze(1)
            extension = ksi_plus_eta_norm - ksi_norm
            ...
            abs_err = np.sqrt(torch.mean(torch.norm(L+f_test[i,:,:,:], dim=-1)**2))
            # # abs_err = np.sqrt(torch.mean(torch.norm(L, dim=-1)**2))
            # B_norm[i] = np.sqrt(torch.mean(torch.norm(data_f[i,:,:,:], dim=-1)**2))
            err[i] = abs_err/B_norm_test[i]
