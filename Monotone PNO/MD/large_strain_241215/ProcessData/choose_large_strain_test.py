...
def main():
    ...
    if if_compute_lambda == 1:
        ...
        # output_file_name = "%s/%s_%s_%s_%s.mat" % (current_dir, data_name, deformation_1, deformation_2)
        # savemat(output_file_name, {'coords': coords, "mass": mass, "disps": disps, "forces": forces})
        
        output_file_name = "%s/index_%s_%s_%s.mat" % (current_dir, data_name, deformation_1, deformation_2)
        savemat(output_file_name, {'index': indices0[0]})
        
        plt_strain(indices0[0],lambda_min_max, "%s/figure/%s_%s_%s_lambda.png" % (current_dir, data_name, deformation_1, deformation_2))
