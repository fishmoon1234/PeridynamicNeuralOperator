...
def parse_layer_info(layer_info):
    ...
    phi_layers_list = [2] + [phi_width] * phi_layers + [1]
    
    return ker_layers_list, phi_layers_list

def plt_b_u_test(u, u_true, y, y_true, base_dir, j, name):
    
    data = [
        [u[:, :, 0], u_true[:, :, 0], np.abs(u_true[:, :, 0] - u[:, :, 0]), 
        y[:, :, 0], y_true[:, :, 0], np.abs(y_true[:, :, 0] - y[:, :, 0])],
        [u[:, :, 1], u_true[:, :, 1], np.abs(u_true[:, :, 1] - u[:, :, 1]), 
        y[:, :, 1], y_true[:, :, 1], np.abs(y_true[:, :, 1] - y[:, :, 1])]
    ]

    titles = ['Predicted', 'Ground truth', 'Absolute error', 'Predicted', 'Ground truth', 'Absolute error']

    plt.rcParams.update({'font.size': 15})
    fig, axes = plt.subplots(2, 6, figsize=(19, 5))

    ims = []  # store imshow() mappable objects

    for row in range(2):
        for col in range(6):
            # decide which norm to use
            if col in [0, 1]:  # first two columns share one colorbar
                norm = Normalize(vmin=min(np.nanmin(data[row][0]), np.nanmin(data[row][1])), 
                                vmax=max(np.nanmax(data[row][0]), np.nanmax(data[row][1])))
            elif col in [3, 4]:  # columns 4 and 5 share one colorbar
                norm = Normalize(vmin=min(np.nanmin(data[row][3]), np.nanmin(data[row][4])), 
                                vmax=max(np.nanmax(data[row][3]), np.nanmax(data[row][4])))
            else:  # remaining columns use their own colorbar
                norm = None
            
            im = axes[row, col].imshow(data[row][col], norm=norm, interpolation='spline16')
            # im = axes[row, col].imshow(data[row][col], interpolation='spline16')
            ims.append(im)
    ...
    # # add colorbar to the first row to avoid duplication
    for row in range(2):
        # columns 0 and 1 share a colorbar placed to the right of column 1
        divider = make_axes_locatable(axes[row, 1])
        cax = divider.append_axes("right", size="5%", pad=0.05)
        fig.colorbar(ims[row * 6], cax=cax)  # column-0 imshow() of the current row

        # column 2 uses its own colorbar
        divider = make_axes_locatable(axes[row, 2])
        cax = divider.append_axes("right", size="5%", pad=0.05)
        fig.colorbar(ims[row * 6 + 2], cax=cax)  # column 2 of the current row

        # columns 3 and 4 share a colorbar placed to the right of column 4
        divider = make_axes_locatable(axes[row, 4])
        cax = divider.append_axes("right", size="5%", pad=0.05)
        fig.colorbar(ims[row * 6 + 3], cax=cax)  # column 3 of the current row

        # column 5 uses its own colorbar
        divider = make_axes_locatable(axes[row, 5])
        cax = divider.append_axes("right", size="5%", pad=0.05)
        fig.colorbar(ims[row * 6 + 5], cax=cax)  # column 5 of the current row

    plt.tight_layout()
    plt.savefig('%s/MD_%s_b_%s.png' % (base_dir, name, j), format='png')
    plt.show()

    
    # data = [
    #     [u[:, :, 0], u_true[:, :, 0], np.abs(u_true[:, :, 0] - u[:, :, 0]), y[:, :, 0], y_true[:, :, 0], np.abs(y_true[:, :, 0] - y[:, :, 0])],
    #     [u[:, :, 1], u_true[:, :, 1], np.abs(u_true[:, :, 1] - u[:, :, 1]), y[:, :, 1], y_true[:, :, 1], np.abs(y_true[:, :, 1] - y[:, :, 1])]
    # ]
    
    # titles = [
    #     'Learned', 'True', 'Absolute error', 'Learned', 'True', 'Absolute error'
    # ]
    
    # # norm = np.zeros((2,6))
    # # for i in range(2):
    # #     norm[i,0] = Normalize(vmin=min(data[i][0].min(), data[i][1].min()), vmax=max(data[i][0].max(), data[i][1].max()))
    # #     norm[i,1] = norm[i,0]
    # #     norm[i,3] = Normalize(vmin=min(data[i][3].min(), data[i][4].min()), vmax=max(data[i][3].max(), data[i][4].max()))
    # #     norm[i,4] = norm[i,3]
    ...
    #         if col in [0, 1]:
    #             # norm_i = norm[row,col]
    #             norm = Normalize(vmin=min(data[row][0].min(), data[row][1].min()), vmax=max(data[row][0].max(), data[row][1].max()))
    #         elif col in [3, 4]:
    #             norm = Normalize(vmin=min(data[row][3].min(), data[row][4].min()), vmax=max(data[row][3].max(), data[row][4].max()))  
    #         else:
    #             norm = None 
            
    #         im = axes[row, col].imshow(data[row][col], norm=norm, interpolation='spline16')
    #         # im = axes[row, col].imshow(data[row][col], extent=[-domain, domain, -domain, domain], interpolation='spline16')
    #         if row == 0:
    #             axes[row, col].set_title(titles[col])
    #         # if col == 2 or col == 5:
    ...
    #     divider = make_axes_locatable(axes[row, 1])
    #     cax = divider.append_axes("right", size="5%", pad=0.05)
    #     fig.colorbar(axes[row, 0].images[0], cax=cax)
        
    #     divider = make_axes_locatable(axes[row, 2])
    #     cax = divider.append_axes("right", size="5%", pad=0.05)
    #     fig.colorbar(axes[row, 2].images[0], cax=cax)
        
    #     divider = make_axes_locatable(axes[row, 4])
    #     cax = divider.append_axes("right", size="5%", pad=0.05)
    #     fig.colorbar(axes[row, 3].images[0], cax=cax)
        
    #     divider = make_axes_locatable(axes[row, 5])
    #     cax = divider.append_axes("right", size="5%", pad=0.05)
    #     fig.colorbar(axes[row, 5].images[0], cax=cax)
...
#     data = [
#         [y[:, :, 0], y[:, :, 1], u[:, :, 0], u[:, :, 1]],
#         [y_true[:, :, 0], y_true[:, :, 1], u_true[:, :, 0], u_true[:, :, 1]],
#         [np.abs(y_true[:, :, 0] - y[:, :, 0]), np.abs(y_true[:, :, 1] - y[:, :, 1]), np.abs(u_true[:, :, 0] - u[:, :, 0]), np.abs(u_true[:, :, 1] - u[:, :, 1])]
#     ]
    
#     titles = [
#         [ r'$b_1$', r'$b_2$', r'$u_1$', r'$u_2$'],
#         [ r'True $b_1$', r'True $b_2$', r'True $u_1$', r'True $u_2$'],
#         [ r'Absolute error of $b_1$', r'Absolute error of $b_2$', r'Absolute error of $u_1$', r'Absolute error of $u_2$']
#     ]
...
def plt_b_u(u, u_true, y, y_true, base_dir, j, name):
    
    data = [
        [y[:, :, 0], y[:, :, 1], u[:, :, 0], u[:, :, 1]],
        [y_true[:, :, 0], y_true[:, :, 1], u_true[:, :, 0], u_true[:, :, 1]],
        [torch.abs(y_true[:, :, 0] - y[:, :, 0]), torch.abs(y_true[:, :, 1] - y[:, :, 1]), torch.abs(u_true[:, :, 0] - u[:, :, 0]), torch.abs(u_true[:, :, 1] - u[:, :, 1])]
    ]
    ...
    # fig.suptitle(r'index=%s, rel $L^2$ error=%s' % (j, err), fontsize=25)
    plt.tight_layout()
    plt.savefig('%s/MD_%s_b_%s.png' % (base_dir, name, j), format='png')
    plt.close()
  
    
# def plt_b_u(u, y_true, y, base_dir, j, err, name):
    
#     data = [
#         [u[:, :, 0], y[:, :, 0], y_true[:, :, 0], torch.abs(y_true[:, :, 0] - y[:, :, 0])],
#         [u[:, :, 1], y[:, :, 1], y_true[:, :, 1], torch.abs(y_true[:, :, 1] - y[:, :, 1])]
#     ]
    
#     titles = [
#         [r'$u_1$', r'$b_1$', r'True $b_1$', r'Absolute error of $b_1$'],
#         [r'$u_2$', r'$b_2$', r'True $b_2$', r'Absolute error of $b_2$']
#     ]
...
#     for row in range(2):
#         for col in range(4):
#             im = axes[row, col].imshow(data[row][col])
#             axes[row, col].set_title(titles[row][col])
#             fig.colorbar(im, ax=axes[row, col])
    
    
#     fig.suptitle(r'index=%s, rel $L^2$ error=%.4f' % (j, err), fontsize=25)
#     plt.tight_layout()
#     plt.savefig('%s/MD_%s_b_%s.png' % (base_dir, name, j), format='png')
    plt.close()
    
    
def plt_b(y_ture, y, base_dir, j, err, name):
    
    fig, axes = plt.subplots(2, 3, figsize=(16, 9))
    im0 = axes[0, 0].imshow(y[:,:,0])
    axes[0, 0].set_title('b1')
    fig.colorbar(im0, ax=axes[0,0])
    im1 = axes[0,1].imshow(y_ture[:,:,0])
    axes[0,1].set_title('True b1')
    fig.colorbar(im1, ax=axes[0,1]) 
    im2 = axes[0,2].imshow(torch.abs(y_ture[:,:,0]-y[:,:,0]))
    axes[0,2].set_title('Absolute error')
    fig.colorbar(im2, ax=axes[0,2]) 
    
    im3 = axes[1, 0].imshow(y[:,:,1])
    axes[1, 0].set_title('b2')
    fig.colorbar(im3, ax=axes[1,0])
    im4 = axes[1,1].imshow(y_ture[:,:,1])
    axes[1,1].set_title('True b2')
    fig.colorbar(im4, ax=axes[1,1]) 
    im5 = axes[1,2].imshow(torch.abs(y_ture[:,:,1]-y[:,:,1]))
    axes[1,2].set_title('Absolute error')
    fig.colorbar(im5, ax=axes[1,2])
    
    fig.suptitle(r'index=%s, rel $L^2$ error=%s' % (j, err))
    plt.tight_layout()
    plt.savefig('%s/MD_%s_b_%s.png' % (base_dir, name, j), format='png')
    plt.close()
...
class LpLoss:
    ...
    def rel(self, x, y):
        ...
        diff_norms = torch.norm(x.reshape(num_examples, -1) - y.reshape(num_examples, -1), self.p, 1)**2
        y_norms = torch.norm(y.reshape(num_examples, -1), self.p, 1)**2 + 1e-8
...
class SquareMeshGenerator:
    ...
    def ball_connectivity(self, r):
        ...
        # self.edge_index = np.vstack((np.where(pwd <= r)) & (np.where(pwd >= 1e-6)))
