...
def plot_u_3(x, u_true, u, u_linear, j, base_dir, name):
    ...
    ax1.plot(x, u_linear, color='green', label='Linear solution', linewidth=2.0)
    ...
    ax1.legend() 
    plt.tight_layout()
    plt.savefig('%s/%s_u_%s.png' % (base_dir, name, j), format='png', bbox_inches='tight',dpi=300)
    plt.close()
...
def plot_err_u_jump(err_u, jump, base_dir, name):
    ...
    plt.tight_layout()
    plt.savefig('%s/%s_err_u_jump.png' % (base_dir, name), format='png', bbox_inches='tight',dpi=300)
    plt.close()
