...
if if_plot == 1:
    ...
    # Plot ground truth and MLP
    ax.plot(Lambdaa, k1_true, color='black', linewidth=linewidth+0.5, 
            label='Ground truth', zorder=4, alpha=0.9)
    ax.plot(Lambdaa, k1_NN, color=colors[1], linestyle='-.', linewidth=linewidth, 
            label='Generic NN', marker='s', markevery=10, markersize=markersize-2, zorder=2)
    ...
    # Enhanced legend
    ax.legend(loc='upper left', bbox_to_anchor=(0.02, 0.98), fontsize=fontsize-6, framealpha=0.95, fancybox=True, shadow=True)
    ...
    plt.savefig('%s/%s_g_NN_%s.png' % (current_dir, ex, layer_info), 
