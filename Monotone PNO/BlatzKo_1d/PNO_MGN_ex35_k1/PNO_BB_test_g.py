...
for i in range(3,N):
    ...
    # base_dir = 'Results/%s_%s_ntrain_%s_lrs_%s_lr_%s_%s' % (ex, layer_info, ntrain, lrs, lr, i)
...
for i in range(3,N):
    ...
    plt.plot(lambdaa, k1_all[0], color=colors[i], linewidth=1.5, label=r'$h=2^{-%s}$' % (i+i0))
# for i in [0,2,4,6]:
...
# plt.yscale('log')
plt.title(r'MGN learned $g(\lambda)$')
plt.grid(True, which="both", ls="--", color='gray')
plt.legend()
plt.tight_layout()
name = '%s_g_%s_ood_%s_%s.png' % (ex, layer_info, lambda_min, lambda_max)
