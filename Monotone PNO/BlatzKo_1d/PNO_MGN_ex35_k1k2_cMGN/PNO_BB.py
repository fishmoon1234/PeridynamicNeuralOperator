...
def parse_layer_info(layer_info):
    ...
    phi_layers_list = [1] + [phi_width] * phi_layers + [1]
...
def main():
    ...
    parser.add_argument('--layer_info', type=str, help='Layer information', default='128_4_128_4')
    parser.add_argument('--test_type', type=str, help='Test type', default='ID')
    ...
    for index in range(3,4):
        ...
        # # layer_info = '128_4_64_4'
        # # phi_2_layer = [1, 64, 64, 64, 64, 1]
        # # layer_info = '128_4_128_5'
        # # phi_2_layer = [1, 128, 128, 128, 128, 128, 1]
        # layer_info = '128_6_128_4'
        # phi_2_layer = [1, 128, 128, 128, 128, 1]
        # layer_info = '128_6_256_6'
        # phi_2_layer = [1, 256, 256, 256, 256, 256, 256, 1]
        # embed_dim = 128
        # num_layers = 6
        
        layer_info = args.layer_info
        embed_dim, num_layers, phi_2_layer = parse_layer_info(layer_info)
        # start_layer = 8
        # layer_add_times = 2
        model = E_GCL_GKN(num_layers, 1, embed_dim, phi_2_layer, nn.ReLU(), nn.Sigmoid()).to(device)
        
        # trained_model = '%s/Results/ex32_128_4_128_4_ntrain_300_lrs_[0.005]_lr_[0.995, 0.998]_sigmoid_gap_1/model_4.ckpt' % current_dir
                ...
                if train_loss / ntrain /s < best_train_loss:
                    with torch.no_grad():
                        ...
                        for batch in test_loader:
                            ...
                            # test_l2 += myloss(out.view(_test, -1), f_gt.view(batch_size_test, -1)).item()
        ...
        ################################################################
            ...
            model_path = os.path.join(base_dir, 'model_4.ckpt')
            ...
            if test_type == 'ID':
                ...
                data_u_test = data_u[:ntrain, :]
                data_f_test = data_f[:ntrain, :]
                data_test = generate_data_with_ksi_eta(data_x, data_u_test, data_f_test, m_fact, dx, s)
            ...
            test_solver(data_train, ntrain, base_dir, 'train')
        
        
            # # model_path = '%s/Results/ex32_128_4_128_4_ntrain_300_lrs_[0.001]_lr_[0.995, 0.998]_sigmoid_gap_1/model_%s.ckpt' % (current_dir, num_layers)
            # model_path = '%s/Results/ex32_128_6_256_6_ntrain_300_lrs_[0.005]_lr_[0.995, 0.998]_sigmoid_gap_2/model_%s.ckpt' % (current_dir, num_layers)
            ...
            # tol_cg, k_cg_max, tol_ls, i_ls_max = 1e-6, 100, 1e-6, 100
