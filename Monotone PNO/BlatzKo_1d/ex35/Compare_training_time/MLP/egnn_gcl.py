...
class E_GCL_GKN(nn.Module):
    ...
    def __init__(self, phi_1_layer, act_fun_xi, normalize=False,
                 coords_agg='mean', residual=True):
        super().__init__()
    ...
    def true_model_nonlinear(self, data):
        u, ksi = data.u, data.ksi
        ndata = u.size(0)
        s, n_ksi = ksi.size()
        eta = torch.zeros((s, n_ksi)).to(u.device)
        ...
        phi_NN2 = torch.exp(-50*ksi_norm**2)*(delta-torch.abs(ksi_norm))
        # phi_NN = (phi_NN1 -phi_NN1_1) *phi_NN2
        tol = torch.tensor(0.5)
        g_tol = torch.pi*(tol - tol**(-3)) + torch.sin(torch.pi*tol)
        g_fun = lambda x: torch.where(x < tol, g_tol, torch.pi*(x - x**(-3)) + torch.sin(torch.pi*x))
        phi_NN1 = g_fun(lambdaa)
        phi_NN = phi_NN1 * phi_NN2
    ...
    def true_model_linear(self, data):
        u, ksi = data.u, data.ksi
        ndata = u.size(0)
        s, n_ksi = ksi.size()
        eta = torch.zeros((s, n_ksi)).to(u.device)
