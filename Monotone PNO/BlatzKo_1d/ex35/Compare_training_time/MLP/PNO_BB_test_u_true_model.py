...
for i in range(3, N):
    ...
    if compute == 1:
        ...
        with torch.no_grad():
            ...
            # for j in range(ndata):
                ...
                def func_linear(u):
                    ...
                    y = model.true_model_linear(data)-data.f.squeeze()
                ...
                def func(u):
                    ...
                    # full_u[s+m_fact] = u_true[s+m_fact]
                    data = Data(x=data_X , u=full_u, f=data_f[j,m_fact:s+m_fact,:], ksi=data_ksi[0, :, :]).to(device)
                    y = model.true_model_nonlinear(data)-data.f.squeeze()
                    return y.cpu().detach().numpy()
        ...
        with open("%s/%s_err_u_record_%s_true_model.txt" % (base_dir, ex_data, initial_type), "a") as file:
