...
class E_GCL_GKN(nn.Module):
    ...
    def __init__(self, phi_1_layer, act_fn=nn.Softplus(), normalize=False,
        ...
        self.icnn = ICNN_init(phi_in_dim, phi_out_dim, phi_width, phi_n_layers,
                convex = "icnn", fix_init= False, activation=self.act_fn, skip=False, s2d=False)
        
        # Cache for phi_NN1_1 computation to avoid redundant calculations
        self.cached_phi_NN1_1 = None
    ...
    def forward(self, data):
        ...
        batch_size = lambdaa.shape[0]
        total_size = batch_size * n_ksi
        
        lambdaa_input = lambdaa.reshape(-1,1).clone().detach().requires_grad_(True)
        ...
        # For phi_NN1_1, use cached value if available and size matches
        if self.training or self.cached_phi_NN1_1 is None or self.cache_size != total_size:
            lambdaa_ones = torch.ones((total_size, 1), device=lambdaa.device, requires_grad=True)
            ...
            if not self.training:
                self.cached_phi_NN1_1 = phi_NN1_1.detach().clone()
                self.cache_size = total_size
            phi_NN1_1 = phi_NN1_1.reshape(-1, n_ksi)
        else:
            phi_NN1_1 = self.cached_phi_NN1_1.reshape(-1, n_ksi)
    ...
    def nonlinear(self, data):
        ...
        # Calculate derivatives of ICNN with respect to input
        lambdaa_ones = torch.ones_like(lambdaa, requires_grad=True).reshape(-1,1)
        lambdaa_input = lambdaa.reshape(-1,1).clone().detach().requires_grad_(True)
