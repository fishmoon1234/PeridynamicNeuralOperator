import torch
# Enable this for debugging in-place/autograd issues
try:
    import os
...
def main():
    ...
    for index in range(3,4):
        ...
        # model = E_GCL_GKN(phi_1_layer, torch.nn.Softplus()).to(device)
        ...
        base_dir = '%s/Results/%s_%s_ntrain_%s_lrs_%s_lr_%s_gap_%s_%s_seed_%s' % (current_dir, ex, layer_info, ntrain, lrs, lr, gap, act_xi, seed)
                    ...
                    for weight_decay in wds:
                        ...
                        for ep in range(epochs):
                            ...
                            for batch in train_loader:
                                ...
                                optimizer.zero_grad()
                                out = model(batch)
                                out = out.contiguous()  # avoid as_strided views before .view()
                                ...
                                loss_batch = torch.norm(out.view(batch_size, -1) - f_gt.view(batch_size, -1), 1)/torch.norm(f_gt.view(batch_size, -1), 1)
                            ...
                            if train_loss < best_train_loss:
                                ...
                                with torch.enable_grad():
                                    for batch in valid_loader:
                                        ...
                                        valid_loss_batch = torch.norm(out.view(batch_size_valid, -1) - f_gt.view(batch_size_valid, -1), 1)/torch.norm(f_gt.view(batch_size_valid, -1), 1)
                                        
                                        valid_loss += (torch.sum(valid_loss_batch)).item()
                                        
                                        # Explicitly clear computation graph
                                        del out, f_gt, valid_loss_batch
                                    ...
                                    for batch in test_loader:
                                        ...
                                        test_loss_batch = torch.norm(out.view(batch_size_test, -1) - f_gt.view(batch_size_test, -1), 1)/torch.norm(f_gt.view(batch_size_test, -1), 1)
                                        test_loss += (torch.sum(test_loss_batch)).item()
                                        
                                        # Explicitly clear computation graph
                                        del out, f_gt, test_loss_batch
                                        torch.cuda.empty_cache()
                            ...
                            if ep % 50 == 0:
                                print(
                                    ...
                                    f'{(t2 - t1):.2f}s, train err:{(train_loss):.4e}, valid err: {(valid_loss):.4e}, test err: {(test_loss):.4e}, '
                        ...
                        print("-" * 100)
                        print("-" * 100)
                        print(f'>> {layer_info}, {act_xi}, ntrain: {ntrain}, lr: {learning_rate}, gamma: {scheduler_gamma}, w_d: {weight_decay}')
                        ...
                        f.write(f'{layer_info}, {act_xi}, {ntrain}, {lrs}, {lr}, {dx}: ')
                        f.write(','.join(f'{err:.4e}' for err in bl_train))
                        f.write(',')
                        f.write(','.join(f'{err:.4e}' for err in bl_valid))
                        f.write(',')
                        f.write(','.join(f'{err:.4e}' for err in bl_test))
        ...
        ################################################################
            ...
            for batch in test_loader:
                ...
                if if_plot == 1:
                    ...
                    plt.savefig('%s/%s_b%s_ICNN.png' % (base_dir, ex, j), format='png', bbox_inches='tight',dpi=300)
                    plt.close()
                
                j += 1
            print(f'>> Mean error of b: {np.mean(err):.2e}')
            ...
            if if_plot_g == 1:
                ...
                g_fun = lambda x: np.pi*(x - x**(-3)) + np.sin(np.pi*x)
                
                Nlambda = 50
                # Lambda_rho = torch.linspace(0.97,3.25, Nlambda)
                Lambda_rho = torch.linspace(0.5,6, Nlambda)
                Lambdaa = Lambda_rho.reshape(-1,1)
                Lambdaa_cuda = Lambdaa.to('cuda')
                Lambdaa_1_cuda = torch.ones_like(Lambdaa_cuda)
                # g_true = 2*c*(Lambda_rho-Lambda_rho**(-3)).reshape(-1,1)
                g_true = g_fun(Lambdaa)
                # Calculate g_NN using ICNN derivatives (phi_NN1 - phi_NN1_1)
                Lambdaa_cuda.requires_grad_(True)
                Lambdaa_1_cuda.requires_grad_(True)
                
                # Forward pass through ICNN for Lambdaa_cuda
                icnn_output = model.icnn(Lambdaa_cuda)
                ...
                icnn_output_1 = model.icnn(Lambdaa_1_cuda)
                ...
                g_NN = (phi_NN1 - phi_NN1_1).reshape(-1,1)
                g_NN = g_NN.cpu().detach()
                
                err = torch.norm(g_NN-g_true)/torch.norm(g_true)
                ...
                plt.plot(Lambdaa, g_NN, color='darkorange', linestyle='--', linewidth=2, label='ICNN')
                plt.plot(Lambdaa, g_true, color='k', linewidth=2, label='Ground Truth')
                plt.legend()
                plt.xlabel(r'$\lambda$', fontsize=fontsize)
                plt.ylabel(r'$g(\lambda)$', fontsize=fontsize)
                plt.savefig('%s/%s_g_%s.png' % (base_dir, ex, dx), format='png', bbox_inches='tight',dpi=300)
                plt.close()
