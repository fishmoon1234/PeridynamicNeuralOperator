...
for i in range(3, N):
    ...
    base_dir = 'Results/%s_%s_ntrain_%s_lrs_%s_lr%s_gap_%s' % (ex_train, layer_info, ntrain, lrs, lr, gap)
    ...
    # plot_index = [3, 17, 22, 36, 38, 41, 51, 62, 75]   
    compute, plot = 1, 0
    if compute == 1:
        ...
        with open("%s/err_u_record.txt" % (current_dir), "a") as file:
            file.write(f"{ex_data}, {initial_type}, {layer_info}, {lrs}, {dx}, {torch.mean(err_u)}\n")