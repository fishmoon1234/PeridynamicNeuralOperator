...
layer_info = '256_6'
phi_2_layer = [1, 256, 256, 256, 256, 256, 256, 1]
lrs = [0.001]
lr = [0.992, 0.998]

model = E_GCL_GKN(phi_2_layer, torch.nn.ReLU()).to(device)
...
# lrs = [0.001]
best_train_loss = [3.4595643762231867e-05, 2.3319597911359865e-07,3.88555835823797e-09,8.467338423080852e-10]
best_test_loss = [4.250387591268063e-06, 2.3813940263050656e-08,4.202404438059748e-10,9.302782571354257e-11]

# lrs = [0.0005]
# best_train_loss = [3.4595643762231867e-05, 2.3319597911359865e-07,3.88555835823797e-09,7.029009928065049e-11]
# best_test_loss = [4.250387591268063e-06, 2.3813940263050656e-08,4.202404438059748e-10, 8.378585714656626e-12]

# lrs = [0.0001]
# best_train_loss = [3.4595643762231867e-05, 2.3319597911359865e-07,3.88555835823797e-09,1.5271042916954317e-10]
# best_test_loss = [4.250387591268063e-06, 2.3813940263050656e-08,4.202404438059748e-10, 2.0715346798805386e-11]
...
for i in range(N):
    ...
    # if i == 3:
    #     lrs = [1e-4]
    #     best_train_loss[i] = 1.5271042916954317e-10
    #     best_test_loss[i] = 2.0715346798805386e-11
        # lrs = [1e-4]
        # best_train_loss[i] = 1.5271042916954317e-10
        # best_test_loss[i] = 2.0715346798805386e-11
    #     lr = [0.992, 0.998]
    ...
    #     # layer_info = '256_5'
    #     # phi_2_layer = [1, 256, 256, 256, 256, 256, 1]
    #     layer_info = '128_6'
    #     phi_2_layer = [1, 128, 128, 128, 128, 128, 128, 1]
    #     layer_info = '256_6'
    #     phi_2_layer = [1, 256, 256, 256, 256, 256, 256, 1]
    #     model = E_GCL_GKN(phi_2_layer, torch.nn.ReLU()).to(device)
    ...
    # rho0 = (torch.flip(rho0, dims=[0])+ rho0)/2
...
plt.title(r'(Ex-II) Learned $k(\xi)$')
plt.legend(loc='best')
ax.grid(True)
plt.tight_layout()
# plt.savefig('%s/%s_k_h_%s_%s.png' % (base_dir, ex, h[i], act_xi), format='png')
plt.savefig('%s/%s_k_%s_%s.png' % (current_dir, ex, layer_info, lrs), format='png', dpi=300)
plt.close()
print('---')
   
 
print("relative L2 rho error for k2: %s" % rel_L2_rho_err_k2)
print(loss_test)
...
# ax1.plot(h, L2_err_k2, 's--', markerfacecolor='none',color=color_err, linewidth=2)
# ax1.plot(h, rel_L2_err_k2, 's:', markerfacecolor='none',color=color_err, linewidth=2)
# ax1.plot(h, L2_rho_err_k2, 's--', markerfacecolor='none',color='orange', linewidth=2)
...
color_loss = 'tomato'
ax2 = ax1.twinx() 
ax2.plot(h, best_train_loss, 'x--', color=color_loss, linewidth=2)
# ax2.plot(h, loss_test, 'x--', color=color_loss, linewidth=2)
ax2.set_yscale('log')
...
plt.tight_layout()
name = '%s_error_k_%s_%s.png' % (ex, layer_info, lrs)
