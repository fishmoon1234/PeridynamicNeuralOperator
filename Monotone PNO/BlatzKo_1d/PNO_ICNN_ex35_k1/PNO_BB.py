                        ...
                        for ep in range(epochs):
                            ...
                            if train_l2 < best_train_loss:
                                ...
                                with torch.enable_grad():
                                    for batch in valid_loader:
                                        ...
                                        out = model(batch)
                                        #out = f_normalizer.decode(out)
                                        out = out.reshape(batch_size2, -1)
                                        # crop 2delta layer from computed force
                                        out = out.view(batch_size2, s, 1)
                                        out = out.view(-1, 1)
                                        #f_gt = f_normalizer.decode(batch.f.view(batch_size, -1))
                                        f_gt = batch.f.view(batch_size2, -1)
                                        f_gt = f_gt.view(batch_size2, s, 1)
                                        f_gt = f_gt.view(-1, 1)
                                    ...
                                    for batch in test_loader:
                                        ...
                                        test_l2 += torch.sum((out.view(batch_size2, -1)-f_gt.view(batch_size2, -1))**2).item()
                                        

                                valid_l2 = valid_l2/nvalid/s
                                test_l2 = test_l2/ntest/s
                            ...
                            if ep % 50 == 0:
                                print(
                                    ...
                                    f'{(t2 - t1):.2f}s, train err:{(train_l2):.4e}, valid err: {(valid_l2):.4e}, test err: {(test_l2):.4e}, '
