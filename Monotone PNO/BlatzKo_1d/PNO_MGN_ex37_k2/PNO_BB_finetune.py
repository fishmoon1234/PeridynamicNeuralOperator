...
def main():
    ...
    for index in range(3,4):
        ...
        trained_model = '%s/Results/ex33_256_4_ntrain_300_lrs_[0.006]_lr[0.993, 0.998]_gap_1' % (current_dir)
        model.load_state_dict(torch.load('%s/model.ckpt' % trained_model, map_location=device))
        # model.load_state_dict(torch.load('%s/../../Hetero_u/round2_trainALL/Results/%s/model.ckpt' % (current_dir,hetero_u_model_name), map_location=device))
