...
def plot_u_error(x, u_true, u, j, base_dir, name, title_name):
    plt.rcParams.update({'font.size': 22}) 
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(10, 5))
    
    # First subplot - solution comparison
    ax1.plot(x, u_true, color='black', label=r'True $u$', linewidth=2.0)
    ax1.plot(x, u, color='orange', label=r'Solved $u$', linewidth=1.2)
    ax1.set_xlabel(r'$x$')
    ...
    ax2.plot(x, np.abs(u_true-u), color='palevioletred', label=r'Error of $u$', linewidth=2.0)
    ax2.set_xlabel(r'$x$')
    ax2.set_ylabel(r'Error', color='black')
    ax2.tick_params(axis='y', colors='black')
    ax2.legend(loc='upper right')
    
    # Common title with reduced padding
    # fig.suptitle('Initial value: %s' % title_name, y=0.98)
    fig.suptitle(title_name, y=0.98)
    # plt.tight_layout() 
    plt.tight_layout(rect=[0, 0, 1, 1.05])  # Adjust the rect to bring title closer to plot
    plt.savefig('%s/%s_error_u_%s.png' % (base_dir, name, j), format='png')
    plt.close()

# def plot_u_error(x, u_error, j, base_dir, name, title_name):
#     plt.rcParams.update({'font.size': 20}) 
#     fig, ax1 = plt.subplots(figsize=(6, 5))
#     ax1.plot(x, u_error, color='palevioletred', label=r'Error of $u$', linewidth=2.0)
#     ax1.set_xlabel(r'$x$')
#     ax1.set_ylabel(r'u(x)', color='black')
#     ax1.tick_params(axis='y', colors='black')
#     ax1.legend(loc='upper right') 
#     plt.title('Initial value: %s' % title_name)
#     plt.tight_layout()
#     plt.savefig('%s/%s_error_u_%s.png' % (base_dir, name, j), format='png')
#     plt.close()

def plot_u(x, u_true, u, j, base_dir, name, title_name):
    plt.rcParams.update({'font.size': 20}) 
    fig, ax1 = plt.subplots(figsize=(6, 5))
    ax1.plot(x, u_true, color='black', label=r'True $u$', linewidth=2.0)
    ax1.plot(x, u, color='orange', label=r'Solved $u$', linewidth=1.2)
...
class LpLoss:
    ...
    def rel(self, x, y):
        ...
        y_norms = torch.norm(y.reshape(num_examples, -1), self.p, 1) + 1e-7
