...
for i in range(N):
    ...
    Lambdaa_1_cuda = torch.ones_like(Lambdaa_cuda)
    dw = ((model.phi_MGN(Lambdaa_cuda)-model.phi_MGN(Lambdaa_1_cuda)) *model.phi_2(Xi_norm_cuda)).reshape(Nxi, Nlambda)
...
plt.plot(h, (h/h[-1])*rel_L2_err_k[-1], 'k', label='slope=1', linewidth=2)
...
plt.tight_layout()
name = '%s_error_gk_%s_%s.png' % (ex, lambda_min, layer_info)
...
for i in range(N):
    plt.plot(xi_norm, k2_all[i], color=colors[i], linewidth=1.5, label=r'$h=2^{-%s}$' % (i+i0))
plt.xlabel(r'$\lambda$', fontsize=fontsize)
