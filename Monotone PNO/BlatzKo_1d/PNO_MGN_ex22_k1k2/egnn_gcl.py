...
#     def __init__(self, nn_1, nn_2, alpha, act_fn=torch.nn.ReLU()):
#         super().__init__()

#         # kernel = DenseNet([ker_in, ker_width//2, ker_width, ker_out], torch.nn.ReLU)
#         # self.egkn_conv = E_GCL_GKN(phi_in, phi_width, phi_out, kernel, alpha, act_fn=act_fn)
#         NN_1 = DenseNet(nn_1, torch.nn.ReLU)
#         NN_2 = DenseNet(nn_2, torch.nn.ReLU)
#         self.egkn_conv = E_GCL_GKN(NN_1, NN_2, alpha, act_fn=act_fn)
...
class E_GCL_GKN(nn.Module):
    ...
    def __init__(self, phi_1_layer, phi_2_layer, act_fun_xi, act_fn=nn.Softplus(), normalize=False,
