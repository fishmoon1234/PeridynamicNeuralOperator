...
def plot_u_3(x, u_true, u, u_linear, j, base_dir, name):
    ...
    ax1.legend() 
    plt.tight_layout()
    plt.savefig('%s/%s_u_%s.png' % (base_dir, name, j), format='png', dpi=300)
    plt.close()
    
def plot_b(x, u_true, u, j, base_dir, name):
    plt.rcParams.update({'font.size': 25}) 
    fig, ax1 = plt.subplots(figsize=(6, 5))
    ax1.plot(x, u_true, color='black', label=r'True')
    ax1.plot(x, u, color='forestgreen', label=r'Predicted', linewidth=1.5)
    ax1.set_xlabel(r'$x$')
    ax1.set_ylabel(r'$b(x)$', color='black')
    ax1.tick_params(axis='y', colors='black')
    ax1.legend() 
    plt.tight_layout()
    plt.savefig('%s/%s_b_%s.png' % (base_dir, name, j), format='png', dpi=300)
    plt.close()
...
def plot_u(x, u_true, u, j, base_dir, name):
    ...
    # Left subplot: u and u_true comparison
    ax1.plot(x, u_true, color='black', label=r'True $u$', linewidth=2.0)
    ax1.plot(x, u, color='orange', label=r'Solved $u$', linewidth=1.5, linestyle='--')
    ax1.set_xlabel(r'$x$', fontsize=22)
    ax1.set_ylabel(r'$u(x)$', fontsize=22)
    ax1.tick_params(axis='both', labelsize=18)
    ax1.legend(fontsize=18)
    ax1.grid(True, alpha=0.3, linestyle='-', linewidth=0.5)
    ax1.set_title('Solution Comparison', fontsize=22, pad=15)
    
    # Right subplot: error plot
    ax2.plot(x, error, color='red', linewidth=2.0)
    ax2.axhline(y=0, color='gray', linestyle='-', alpha=0.5, linewidth=1)
    ax2.set_xlabel(r'$x$', fontsize=22)
    ax2.set_ylabel(r'Error', fontsize=22)
    ax2.tick_params(axis='both', labelsize=18)
    ax2.grid(True, alpha=0.3, linestyle='-', linewidth=0.5)
    ax2.set_title('Solution Error', fontsize=22, pad=15)
