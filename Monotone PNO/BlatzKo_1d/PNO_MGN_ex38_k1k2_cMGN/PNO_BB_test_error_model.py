...
for i in range(3,N):
    ...
    model = E_GCL_GKN(num_layers, 1, embed_dim, phi_2_layer, torch.nn.ReLU(), nn.Sigmoid()).to('cuda')
    ...
    err_gk = torch.norm(k1_NN.reshape(-1,1)*k2_NN.reshape(1,-1)-k1_true.reshape(-1,1)*k2_true.reshape(1,-1))/torch.norm(k1_true.reshape(-1,1)*k2_true.reshape(1,-1))
