...
u_fun = lambda a, b, d, x: (np.sum(a*np.exp(-feq/nfeq)*np.sin(feq*np.pi*x)+ b*np.exp(-feq/nfeq)*np.cos(feq*np.pi*x) , axis=0)+ d*np.atleast_1d(x)).ravel() 
...
while n < N_data:
    ...
    u = u_fun(a, b, d, xh_all.reshape(1,-1))
    ...
    for i in range(m_fact, Nx+m_fact):
        ...
        eta = lambda ksi: u_fun(a, b, d, ksi+xi)- u[i]
        ...
        I1, err1[n,i-m_fact] = integrate.quad(f, 0, delta, epsabs=1e-12, epsrel=1e-12)
        I2, err2[n,i-m_fact] = integrate.quad(f, -delta, 0, epsabs=1e-12, epsrel=1e-12)
        # print(err1)
        # print(err2)
        bodyforce[n,i] = I1+I2
    ...
    print(d, torch.min(lambda_min_data), torch.max(lambda_max_data))
    
    
    if np.max(err1[n,:])<=1e-10 and np.max(err2[n,:])<=1e-10 and (torch.min(lambda_min_data) <= 0.82 or torch.max(lambda_max_data) >= 3.35):
        n = n+1
...
data_name = "BK_ex35_ood_ndata_%s_Nx_%s_delta_%s_h_%s.mat" % (N_data, Nx, delta, h)
