...
def plot_b(out_plot, f_gt_plot, base_dir, ex, n):
    ...
    # Plot b0 (x-component)
    im0 = axes[0, 0].imshow(out_plot[:,:,0], vmin=vmin_b0, vmax=vmax_b0, aspect='equal')
    axes[0, 0].set_title('Predicted $b_0$', fontweight='bold', pad=15)
    cbar0 = fig.colorbar(im0, ax=axes[0,0], shrink=0.75, aspect=20)
    
    im1 = axes[0,1].imshow(f_gt_plot[:,:,0], vmin=vmin_b0, vmax=vmax_b0, aspect='equal')
    axes[0,1].set_title('True $b_0$', fontweight='bold', pad=15)
    cbar1 = fig.colorbar(im1, ax=axes[0,1], shrink=0.75, aspect=20)
    
    error_b0 = f_gt_plot[:,:,0] - out_plot[:,:,0]
    vmax_error_b0 = np.max(np.abs(error_b0))
    im2 = axes[0,2].imshow(error_b0, vmin=-vmax_error_b0, vmax=vmax_error_b0, aspect='equal')
    axes[0,2].set_title('Absolute Error of $b_0$', fontweight='bold', pad=15)
    cbar2 = fig.colorbar(im2, ax=axes[0,2], shrink=0.75, aspect=20)
    
    # Plot b1 (y-component)
    im3 = axes[1, 0].imshow(out_plot[:,:,1], vmin=vmin_b1, vmax=vmax_b1, aspect='equal')
    axes[1, 0].set_title('Predicted $b_1$', fontweight='bold', pad=15)
    cbar3 = fig.colorbar(im3, ax=axes[1,0], shrink=0.75, aspect=20)
    
    im4 = axes[1,1].imshow(f_gt_plot[:,:,1], vmin=vmin_b1, vmax=vmax_b1, aspect='equal')
    axes[1,1].set_title('True $b_1$', fontweight='bold', pad=15)
    cbar4 = fig.colorbar(im4, ax=axes[1,1], shrink=0.75, aspect=20)
    
    error_b1 = f_gt_plot[:,:,1] - out_plot[:,:,1]
    vmax_error_b1 = np.max(np.abs(error_b1))
    im5 = axes[1,2].imshow(error_b1, vmin=-vmax_error_b1, vmax=vmax_error_b1, aspect='equal')
    axes[1,2].set_title('Absolute Error of $b_1$', fontweight='bold', pad=15)
    cbar5 = fig.colorbar(im5, ax=axes[1,2], shrink=0.75, aspect=20)
    ...
    plt.show()
    plt.savefig('%s/2D_%s_b%s_MGN.png' % (base_dir, ex, n), format='png', dpi=300, bbox_inches='tight')
    plt.close()
