#!/usr/bin/env python3
"""Quasistatic fracture simulation of a pre-notched plate with a
peridynamic neural operator (PNO) as the constitutive model.

Constitutive model (per bond):
    lambda = 1 + (|xi + eta| - |xi|)/|xi|
    f_ij = mu_ij g(lambda) k_sym(xi/delta) |xi/delta|^(-alpha)
           (xi+eta)/|xi+eta| dx^2
with the symmetrized kernel k_sym(z) = (k(z)+k(-z))/2, the learned
softening g continued past its tension peak by a C1-matched exponential
tail (decay G_DECAY = 10), critical stretch s0 = lam_peak - 1, and
near-hard bond breaking (BONDSOFT_DECAY = 10000).  The operator
amplitude carries the unit conversion s^2/sigma_f; the horizon is a
model property, delta = gamma * delta_MD, independent of the grid.

Geometry: the plate [-L, L] x [-2L - dx/2, 2L + dx/2] on a staggered
grid with node rows at y = +-dx/2, +-3dx/2, ...  The horizontal
pre-notch [-notch_half, notch_half] x {0} runs BETWEEN the two middle
node layers, and the measurement planes y = 0, +-L likewise fall
between rows, so the cross-section force sum has no on-row special
cases.

Loading and solve: the top/bottom grip rows (|y| >= 2L - strip_width)
prescribe u_y = +-pull_rate * t (x free); the center line x = 0 pins
u_x = 0.  "t" is purely a loading parameter: each step solves the
static equilibrium f_int(u) = 0 on free dofs by a globalized
(line search + Levenberg-Marquardt) matrix-free Newton-Krylov method.
Bond breaking is applied after Newton converges; if the damage mask
grew, the step is re-solved with the updated frozen mask.  Free
surfaces are untreated (--boundary nothing); --boundary traction is
not supported on the staggered grid.

Outputs per step (force.txt, physical MD force units): grip reactions
and the bond force transmitted across y = 0 and y = +-L; damage and
displacement snapshots (PNG + state npz).
...
Usage:
    python fracture_estimate_force.py [options]
"""
...
# full driver):
G_DECAY = 10.0          # exponential decay of g past its peak
BONDSOFT_DECAY = 10000.0  # bondsoft sharpness (near-hard bond break)


# ═══════════════════════════════════════════════════════════════════════#  Learned-g cutoff wrapper (analogous to solve_pno --g-variant c1, but
#  the elastic branch is the learned g rather than lambda - lambda^-3).
#  Past lam_cut, g(lam) decays exponentially with a C^1 match to the
#  learned slope at lam_cut.  When lam_cut is set to the learned peak,
#  the slope at the cut is ~0, giving a pure exp(-k s) g_peak tail.
# ═════════════════════════════════════════════════════════════════════
...
# ═════════════════════════════════════════════════════════════════════#  Model loading (from solver/solve_pno.py)
# ═══════════════════════════════════════════════════════════════════════
...
#  mirror fictitious nodes + per-block shifts p_alpha (Lu-Li
#  zero-traction surfaces)
# ═══════════════════════════════════════════════════════════════════════
...
def plot_bonds_for_row_nodes(x, edge_index, bond_alive, out_dir, y0=0.0,
                             epsilon=1e-8, zoom=None, fname='bonds_row_y0.png'):
    """Visualize EVERY bond incident to a grid node lying on the row
    y = y0 (either endpoint), not just the subset that crosses the line
    (as in ``plot_bonds_across_y0``) -- this also shows bonds that run
    along the row itself and bonds reaching from a row node up/down into
    the bulk, which is useful for inspecting the pre-notch bond pattern
    directly (e.g. to check for directional/asymmetric stencil artifacts
    near the notch tip). Bonds are colored by whether they are still
    intact (``bond_alive``) or already broken.

    ``zoom``, if given, is a (half-width, half-height) tuple restricting
    the plotted x/y range to the row nodes' bounding region expanded by
    that margin (useful for a close-up of the notch tip); otherwise the
    ...
    on_row = np.abs(xc[:, 1] - y0) < epsilon
    incident = on_row[row_n] | on_row[col_n]
    idx = np.nonzero(incident)[0]
    ...
    fig, ax = plt.subplots(figsize=(14, 6))
    ax.scatter(xc[:, 0], xc[:, 1], s=3, color='#DDDDDD', zorder=1)
    ax.scatter(xc[on_row, 0], xc[on_row, 1], s=14, color='#333333', zorder=3,
              label=f'nodes on $y={y0:g}$')
    if segs:
        lc = LineCollection(segs, colors=seg_colors, linewidths=1.2, zorder=2)
        ax.add_collection(lc)
    ax.axhline(y0, color='black', linestyle='--', linewidth=1.0, zorder=0)
    ...
    ax.set_title(f'Bonds incident to nodes on $y={y0:g}$: '
                f'{len(idx)} total, {n_alive} intact')
    if zoom is not None:
        hx, hy = zoom
        row_x = xc[on_row, 0]
        cx = 0.5 * (row_x.min() + row_x.max()) if len(row_x) else 0.0
        ax.set_xlim(cx - hx, cx + hx)
        ax.set_ylim(y0 - hy, y0 + hy)
    plt.savefig(Path(out_dir) / fname, dpi=300, bbox_inches='tight')
    plt.close()
...
def run_simulation(args):
    ...
    lam_cut = lam_peak            # cutoff at the learned peak (fixed)
    g_fn, g_info = build_learned_cutoff_g(model, lam_cut=lam_cut,
                                          decay=G_DECAY, device=device)
    print(f"  cutoff-g (learned + exp tail): decay={g_info['decay']}"
          f"  g_cut={g_info['g_cut']:.4e}"
          f"  slope_at_cut={g_info['slope_at_cut']:.4e}")
    ...
    # number of rows at y = +-dx/2, ..., +-(2L + dx/2); the pre-notch
    # plane y = 0 and the measurement planes y = +-L lie between rows.
    Ly_target = 4.0 * L + dx
    ...
    # ── 5. Pre-notch: [-L/2, L/2] x {0} ───────────────────────────────
    notch_y = 0.0
    notch_half = args.notch_half_length if args.notch_half_length is not None else L / 2.0
    notch_x_start, notch_x_end = -notch_half, notch_half
    bond_alive = initialize_prenotch(x, edge_index, notch_x_start, notch_x_end, notch_y, dx)
    bond_alive = bond_alive.to(device)
    # Continuous, non-increasing strength field -- starts equal to the
    # binary alive mask (so pre-notched bonds have strength 0) and is
    # tapered via bondsoft as bonds break.  This is the multiplier used
    # inside the PNO operator; bond_alive is kept only for counting.
    bond_strength = bond_alive.clone()

    # Diagnostic: nodes with no intact incident bond right after the
    # pre-notch is cut (e.g. interior pre-notch nodes fully surrounded by
    # severed bonds) -- these are excluded from the quasistatic
    # solves entirely (see compute_isolated_nodes) since f_int is
    # identically 0 for them regardless of displacement.
    isolated0 = compute_isolated_nodes(edge_index, bond_alive, N)
    n_isolated0 = int(isolated0.sum().item())
    print(f"  {n_isolated0} isolated node(s) at initial setup (no intact "
          f"incident bonds after pre-notch)")
    if n_isolated0 > 0:
        iso_xy = x[isolated0.cpu()].numpy()
        for k, (xi, yi) in enumerate(iso_xy):
            print(f"    isolated[{k}]: (x={xi:.3f}, y={yi:.3f})")

    # Diagnostic: which bonds compute_force_across_y0 sums over (static,
    # depends only on the reference geometry / pre-notch, not on time).
    plot_bonds_across_y0(x, edge_index, bond_alive, out_dir, y0=notch_y)
    # Diagnostic: every bond incident to a node on the y=0 (notch) row,
    # including bonds running along the row and reaching into the bulk,
    # both full-domain and a close-up zoom around the notch tip.
    plot_bonds_for_row_nodes(x, edge_index, bond_alive, out_dir, y0=notch_y)
    plot_bonds_for_row_nodes(x, edge_index, bond_alive, out_dir, y0=notch_y,
                             zoom=(4 * delta, 4 * delta),
                             fname='bonds_row_y0_zoom.png')
    ...
    # ── Breakable mask (protect boundary bonds from surface-effect fracture)
    if args.protect_boundary:
        row, col = edge_index
        bnd_node = top_strip | bot_strip         # nodes in boundary strip
        edge_touches_bnd = bnd_node[row] | bnd_node[col]
        breakable = (~edge_touches_bnd).to(device)
        print(f"  Boundary protection: ON  "
              f"({edge_touches_bnd.sum().item()} edges protected)")
    else:
        breakable = None
    ...
    if args.boundary == "traction":
        raise SystemExit(
            "ERROR: --boundary traction is not supported on the staggered "
            "grid: the Lu-Li crack-face phantom construction assumes the "
            "crack lies ON a node row, which does not exist when the "
            "crack runs between two layers.  Use --boundary nothing.")
    if args.boundary == "traction":
        ghosts = build_ghosts(x, dx, delta, Lx / 2.0, notch_half,
                              notch_y=notch_y)
        g_rows, g_idx = build_ghost_edges(x, ghosts, delta, notch_half,
                                          notch_y=notch_y,
...
def parse_args():
    ...
    p.add_argument("--notch-half-length", type=float, default=None,
                   help="Half-length of the pre-notch crack at y=0, i.e. the "
                        "notch spans [-notch_half_length, notch_half_length]. "
                        "If omitted, defaults to L/2, giving the notch "
                        "[-L/2, L/2] x {0}.")
    ...
    p.add_argument("--protect-boundary", action="store_true",
                   help="Prevent bond breaking near top/bottom boundary strips")
    ...
    p.add_argument("--out-dir", type=str,
                   default="results_force_fracture",
