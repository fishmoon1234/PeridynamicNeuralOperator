...
class LazyClippedPositivity(Positivity):
    ...
    def __call__(self, weight):
        # Avoid in-place modification; return a positive view for forward
        # If you want hard clipping of parameters, do it after optimizer.step() with no_grad.
        return torch.clamp(weight, min=0)
...
class ConvexLinear(nn.Linear):
    ...
    def forward(self, x):
        Wpos = self.positivity(self.weight)
        return torch.nn.functional.linear(x, Wpos, self.bias)
