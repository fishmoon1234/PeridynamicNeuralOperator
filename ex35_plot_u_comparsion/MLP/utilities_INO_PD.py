...
# def plot_u(x_true, u_true, x, u, j, base_dir, name):
#     plt.rcParams.update({'font.size': 15}) 
#     fig, ax1 = plt.subplots(figsize=(6, 5))
#     ax1.plot(x_true, u_true, color='black', label='True u')
#     ax1.plot(x, u, color='darkviolet', label='Learned u')
#     ax1.set_xlabel(r'$x$')
#     ax1.set_ylabel(r'u(x)', color='black')
#     ax1.tick_params(axis='y', colors='black')
#     ax1.legend() 
#     plt.tight_layout()
#     plt.savefig('%s/%s_u_%s.png' % (base_dir, name, j), format='png')
#     plt.close()

def plot_u_NN_MGN(x, u_true, u_NN, u_MGN, j, base_dir, name):
    plt.rcParams.update({'font.size': 22}) 
    fig, ax1 = plt.subplots(figsize=(6, 5))
    ax1.plot(x, u_true, color='black', label='true', linewidth=2.5)
    ax1.plot(x, u_MGN, color='orange', label='MGN', linewidth=2.5, linestyle='--')
    ax1.plot(x, u_NN, color='darkviolet', label='MLP', linewidth=2.5, linestyle='-.')
    ax1.set_xlabel(r'$x$')
    ax1.set_ylabel(r'$u(x)$', color='black')
    ax1.tick_params(axis='y', colors='black')
    ax1.legend(fontsize=21) 
    plt.tight_layout()
    plt.savefig('%s/%s_u_NN_MGN_%s.png' % (base_dir, name, j), format='png')
    plt.close()

def plot_u_NN_MGN_ICNN(x, u_true, u_NN, u_MGN, u_ICNN, j, base_dir, name):
    ...
    # Use more attractive colors and styles
    colors = ['#FF6B35', '#004E98', '#8E44AD', '#2ECC71', '#F39C12', '#95A5A6'] 
    ax1.plot(x, u_true, color='black', label='True Solution', linewidth=3.0, alpha=0.9)
    ax1.plot(x, u_MGN, color=colors[0], label='MGN', linewidth=2.8, linestyle='--', alpha=0.85)
    ax1.plot(x, u_NN, color=colors[1], label='MLP', linewidth=2.8, linestyle='-.', alpha=0.85)
    ax1.plot(x, u_ICNN, color=colors[2], label='ICNN', linewidth=2.8, linestyle=':', alpha=0.85)
    
    # Enhanced labels and formatting
    ax1.set_xlabel(r'$x$', fontsize=20, fontweight='bold')
    ax1.set_ylabel(r'$u(x)$', fontsize=20, fontweight='bold')
    ax1.tick_params(axis='both', colors='black', labelsize=20)
    
    # Improved legend with frame and shadow
    legend = ax1.legend(
        fontsize=16, 
        loc='best',
        frameon=True,
    ...
    plt.savefig('%s/%s_u_NN_MGN_ICNN_%s.png' % (base_dir, name, j), 
...
def plot_u_3(x, u_true, u, u_linear, j, base_dir, name):
    plt.rcParams.update({'font.size': 15}) 
    fig, ax1 = plt.subplots(figsize=(6, 5))
    ax1.plot(x, u_true, color='black', label='True u')
    ax1.plot(x, u_linear, color='green', label='Linear solution', linewidth=1.5)
    ax1.plot(x, u, color='darkviolet', label='Learned u', linewidth=1.2)
    ax1.set_xlabel(r'$x$')
    ax1.set_ylabel(r'u(x)', color='black')
    ax1.tick_params(axis='y', colors='black')
    ax1.legend() 
...
def plot_err_u_jump(err_u, jump, base_dir, name):
    ...
    plt.rcParams.update({'font.size': 15}) 
    fig, ax1 = plt.subplots(figsize=(6, 5))
    ax1.plot(err_u, color='tomato', label=r'$u$ error')
    ax1.set_xlabel(r'$x$')
    ax1.set_ylabel(r'$u$ error', color='tomato')
    ax1.tick_params(axis='y', colors='tomato')

    ax2 = ax1.twinx()
    ax2.plot(jump, color='black', label=r'initial value jump')
    ax2.set_ylabel(r'initial value jump', color='black')
    ...
    plt.tight_layout()
    plt.savefig('%s/%s_err_u_jump.png' % (base_dir, name), format='png')
    plt.close()
...
def plot_err_u_lambda(err_u, lambda_min_data, lambda_max_data, base_dir, name):
    ...
    ax2 = ax1.twinx()
    ax2.plot(lambda_min_data, linestyle='--', color='black', label=r'min of $\lambda$')
    ax2.plot(lambda_max_data, color='black', label=r'max of $\lambda$')
    ax2.set_ylabel(r'$\lambda$ range', color='black')
    ax2.tick_params(axis='y', colors='black')
    lines1, labels1 = ax1.get_legend_handles_labels()
    lines2, labels2 = ax2.get_legend_handles_labels()
    # ax1.legend(lines1 + lines2, labels1 + labels2, loc='upper left') 
    ax1.legend(lines1 + lines2, labels1 + labels2, fontsize=12) 
    plt.tight_layout()
    plt.savefig('%s/%s_err_u_lambda.png' % (base_dir, name), format='png')
    plt.close()
...
def plot_b_u(x, b_true, b, u_true, u, j, base_dir, name):
    ...
    ax[0].plot(x, b_true, color='black', label='True b')
    ax[0].plot(x, b, color='forestgreen', label='Learned b')
    ax[0].set_xlabel(r'$x$')
    ax[0].set_ylabel(r'b(x)', color='black')
    ax[0].legend()
    ax[1].plot(x, u_true, color='black', label='True u')
    ax[1].plot(x, u, color='darkviolet', label='Learned u')
    ax[1].set_xlabel(r'$x$')
    ax[1].set_ylabel(r'u(x)', color='black')
    ax[1].legend()
    plt.tight_layout()
    plt.savefig('%s/%s_b_u_%s.png' % (base_dir, name, j), format='png')
    plt.close()
...
def plot_u_err(x, u_true, u, j, base_dir, name):
    ...
    ax2 = ax1.twinx()
    ax2.plot(x, torch.abs(u_true-u), color='tomato', label='Error')
    ax2.set_ylabel(r'Error of $u$', color='tomato')
...
def plot_b_err(x, b_true, b, j, base_dir, name):
    plt.rcParams.update({'font.size': 15}) 
    fig, ax1 = plt.subplots(figsize=(6, 5))
    ax1.plot(x, b_true, color='black', label='True b')
    ax1.plot(x, b, color='darkviolet', label='Learned b')
    ax1.set_xlabel(r'$x$')
    ax1.set_ylabel(r'b(x)', color='black')
    ax1.tick_params(axis='y', colors='black')

    ax2 = ax1.twinx() 
    ax2.plot(x, torch.abs(b_true-b), color='tomato', label='Error')
    ax2.set_ylabel(r'Error of $b$', color='tomato')
    ax2.tick_params(axis='y', colors='tomato')
    lines1, labels1 = ax1.get_legend_handles_labels()
    lines2, labels2 = ax2.get_legend_handles_labels()
    ax1.legend(lines1 + lines2, labels1 + labels2, loc='upper left') 
    plt.tight_layout()
    plt.savefig('%s/%s_b_%s.png' % (base_dir, name, j), format='png')
    plt.close()
...
class DenseNet(torch.nn.Module):
    def __init__(self, layers, nonlinearity, out_nonlinearity=None, normalize=False):
        ...
        # If nonlinearity is a class, instantiate it
        if isinstance(nonlinearity, type) and issubclass(nonlinearity, nn.Module):
            nonlinearity = nonlinearity()
        
        # If out_nonlinearity is a class, instantiate it
        if out_nonlinearity is not None and isinstance(out_nonlinearity, type) and issubclass(out_nonlinearity, nn.Module):
