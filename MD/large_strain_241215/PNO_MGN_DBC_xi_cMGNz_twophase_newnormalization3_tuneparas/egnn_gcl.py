...
class E_GCL_GKN(nn.Module):
    ...
    def __init__(self, num_layers, embed_dim, phi_2_layer, act_fun_xi, act_fn, normalize=False, coords_agg='mean', residual=True):
        super().__init__()
    ...
    def forward(self, data):
        ...
        phi_NN = g_NN*k_NN
        # force = dx[0]**2*unsorted_segment_sum(kernel* phi_NN * bond_dir, row, num_segments=x.size(0))
    ...
    def linear(self, data):
        ...
        phi_NN = (self.phi_MGN(lambdaa)-self.phi_MGN(lambdaa_1))* self.phi_2(ksi/delta)
        force = dx**2*unsorted_segment_sum(phi_NN * bond_dir, row, num_segments=x.size(0))
