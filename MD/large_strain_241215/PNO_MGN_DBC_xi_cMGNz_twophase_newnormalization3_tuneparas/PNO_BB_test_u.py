...
def parse_layer_info(layer_info):
    ...
    return ker_width, ker_layers, phi_layers_list
...
def main():
    ...
    parser.add_argument('--lrs', type=float, default='1e-3')
    parser.add_argument('--layer_info', type=str, default='16_16_256_4')
    ...
    embed_dim, start_layer, phi_2_layer = parse_layer_info(layer_info)
    
    model = E_GCL_GKN(start_layer, embed_dim, phi_2_layer, nn.ReLU(), nn.Sigmoid()).to(device)
    ...
    def train_single_layer(model, train_loader, valid_loader, test_loader, model_filename, optimizer, epochs, layer_index):
        ...
        f.write(f'{layer_index}, {DATA_NAME}, {ntrain}, {layer_info}, {learning_rate}, {scheduler_gamma}, {weight_decay}: ')
        ...
        for learning_rate in lrs:
            for scheduler_gamma in gammas:
                for weight_decay in wds:
                    print(f'>> ntrain: {ntrain}, lr: {learning_rate}, gamma: {scheduler_gamma}, w_d: {weight_decay}')
                    base_dir = '%s/Results/%s_nvalid%s_%s_lr%s_dr%s_step%s_bs%s_norm%s_z' % (current_dir,INDEX_NAME, nvalid, layer_info, learning_rate , scheduler_gamma, scheduler_step, batch_size, norm)

                    if not os.path.exists(base_dir):
                        os.makedirs(base_dir)
                    
                    layer_index = start_layer
                    model_filename = '%s/model_%s.ckpt' % (base_dir, layer_index)
                    optimizer = torch.optim.Adam(model.parameters(), lr=learning_rate, weight_decay=weight_decay)
                        
                    train_single_layer(model, train_loader, valid_loader, test_loader, model_filename, optimizer, epochs, layer_index)
                    
                    if if_layer_train == '1':
                        for layer_index in range(start_layer+1, model.phi_MGN.num_modules + 1):
                            print(f"Training with {layer_index} layers...")

                            model.phi_MGN.add_layer()
                            model_filename = '%s/model_%s.ckpt' % (base_dir, layer_index)
    ...
    ################################################################
        ...
        results_name = '%s_nvalid50_%s_lr%s_dr0.5_step100_bs2_norm1_z' % (INDEX_NAME, layer_info, args.lrs)
        base_dir = '%s/Results/%s' % (current_dir, results_name)
        # base_dir = '%s/Results/MD_index_274_nvalid50_16_16_256_4_lr0.001_dr0.5_step100_bs2_norm1_z' % current_dir
        ...
        model_path = os.path.join(base_dir, 'model_%s.ckpt' % start_layer)
        ...
        def test(data, name, ntest):
            ...
            for batch in test_loader:
                ...
                if args.solution_type =='one_phase':
                    ...
                    print(f"{j}, err_b:{err_b_test[j]}, err_u:{err_u_test[j]}, fsolve flag:{ier}, {np.max(np.abs(info['fvec']))}, {info['nfev']}\n")
                else:
                    u_linear, info, ier, msg = fsolve(func_linear, data_u0, xtol=1e-9, full_output=True)
                    u = torch.tensor(u_linear, dtype=torch.float32)
                    err_u_test_linear[j] = torch.norm(u-u_true_i.view(-1,))/torch.norm(u_true_i.view(-1,))
                    print('linear:', f"{j}, err_b:{err_b_test[j]}, err_u:{err_u_test_linear[j]}, fsolve flag:{ier}, {np.max(np.abs(info['fvec']))}, {info['nfev']}")
                    
                    u, info, ier, msg = fsolve(func, u_linear, xtol=1e-9, full_output=True)
                    u = torch.tensor(u, dtype=torch.float32)
                    err_u_test[j] = torch.norm(u-u_true_i.view(-1,))/torch.norm(u_true_i.view(-1,))
                    print('nonlinear:', f"{j}, err_b:{err_b_test[j]}, err_u:{err_u_test[j]}, fsolve flag:{ier}, {np.max(np.abs(info['fvec']))}, {info['nfev']}")
            ...
            print('Relative L2 error for %s b: %s' % (name, mean_error_b) )
            print('Relative L2 error for %s u: %s' % (name, mean_error_u) )
            print('Relative L2 error for %s u_linear: %s' % (name, mean_error_u_linear) )
        ...
        if plot_g == 1:
            ...
            ax.plot(lambdaa, k1_NN, color='deepskyblue', linewidth=4.0, linestyle='--', 
                   alpha=0.9, marker='o', markersize=3, markevery=10, 
                   markerfacecolor='white', markeredgecolor='deepskyblue', markeredgewidth=1.5)
            
            # Add grid for better readability
            ax.grid(True, alpha=0.3, linestyle='-', linewidth=0.5)
            
            # Set labels and title with better positioning
            ax.set_xlabel(r'$\lambda$', fontsize=fontsize, fontweight='bold')
            # ax.set_ylabel(r'$g(\lambda)$', fontsize=fontsize, fontweight='bold')
            ax.set_title(r'$g(\lambda)$', fontsize=fontsize+2, fontweight='bold', pad=20)
            
            # Improve tick formatting
            ax.tick_params(axis='both', which='major', labelsize=fontsize-2, width=1.5, length=6)
            ax.tick_params(axis='both', which='minor', width=1, length=4)
            
            # Set axis limits with some padding
            ax.set_xlim(lambda_min-0.01, lambda_max+0.01)
            
            # Add subtle background color
            ax.set_facecolor('#fafafa')
            
            plt.tight_layout()
            plt.savefig('%s/MD_g.png' % (base_dir), format='png', dpi=300, bbox_inches='tight')
            plt.close(fig)
        ...
        if plot_k == 1:
            ...
            ax.axis('off')
            plt.tight_layout()
            plt.savefig('%s/MD_k_2d.png' % (base_dir), format='png', dpi=300, bbox_inches='tight')
            plt.close()
