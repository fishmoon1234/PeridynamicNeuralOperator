...
def main():
    ...
    parser.add_argument('--solution_type', type=str, help='Type of solution', default='two_phase')
    ...
    # Parse layer_info to generate ker_layers and phi_layers dynamically
    ...
    def train_single_layer(model, train_loader, valid_loader, test_loader, model_filename, optimizer, epochs):
        ...
        for ep in range(epochs):
            ...
            for batch in train_loader:
                ...
                train_loss_1 = myloss(out.view(batch_size, -1), f_gt.view(batch_size, -1))
    ...
    ################################################################
        ...
        base_dir = '%s/Results/MD_index_274_nvalid50_%s_lr0.001_dr0.5_step100_bs1_norm1_z_seed_1_42' % (current_dir, args.layer_info)
        ...
        def test(data, name, ntest):
            ...
            for batch in test_loader:
                ...
                if args.solution_type =='one_phase':
                    ...
                    print(f"{j}, {err_b_test[j]:.2e}, {err_u_test[j]:.2e}, fsolve flag:{ier}, {np.max(info['fvec'])}, {info['nfev']}\n")
                else:
                    ...
                    print('linear:', f"{j}, {err_b_test[j]:.2e}, {err_u_test_linear[j]:.2e}, fsolve flag:{ier}, {np.max(info['fvec'])}, {info['nfev']}")
                    
                    u, info, ier, msg = fsolve(func, u_linear, xtol=1e-6, full_output=True)
                    u = torch.tensor(u, dtype=torch.float32)
                    err_u_test[j] = torch.norm(u-u_true_i.view(-1,))/torch.norm(u_true_i.view(-1,))
                    print('nonlinear:', f"{j}, {err_b_test[j]:.2e}, {err_u_test[j]:.2e}, fsolve flag:{ier}, {np.max(info['fvec'])}, {info['nfev']}")
            ...
            print('Relative L2 error for %s b: %.2e' % (name, np.mean(err_b_test)) )
            print('Relative L2 error for %s u: %.2e' % (name, np.mean(err_u_test)) )
            print('Relative L2 error for %s u_linear: %.2e' % (name, np.mean(err_u_test_linear)) )
        ...
        if plot_g == 1:
            ...
            plt.tight_layout()
            plt.savefig('%s/MD_g.png' % (base_dir), format='png')
            plt.close(fig)
        ...
        if plot_k == 1:
            ...
            k2_NN = (model.phi_2(Xi_cuda)).cpu().detach().numpy().reshape(Nxi, Nxi)
            ...
            ax.axis('off')
            plt.tight_layout()
            plt.savefig('%s/MD_k_2d.png' % (base_dir), format='png')
            plt.close()
