...
def main():
    ...
    if if_compute_lambda == 1:
        ...
        output_file_name = "%s/%s_%s_%s.mat" % (current_dir, data_name, INDEX_NAME, deformation)
        ...
        fig, ax = plt.subplots(figsize = (20,5))
        fontsize = 15
        ax.plot(lambda_min_max[indices0[0],0], color='darkorange', linewidth=2)
        ax.plot(lambda_min_max[indices0[0],1], color='green', linewidth=2)
        plt.title('min and max strain')
        plt.tight_layout()
        # plt.savefig('%s/figure/%s_lambda.png' % (current_dir, data_name), format='png')
        plt.savefig('%s/figure/%s_%s_lambda.png' % (current_dir, INDEX_NAME, deformation), format='png')
        plt.close(fig)
        ...
        deformation_2 = 2
        deformation_3 = 1.1
        indices0 = np.where((lambda_min_max[:, 0] >= (deformation_1)) & (lambda_min_max[:, 1] >= (deformation_3)) & (lambda_min_max[:, 1] <= (deformation_2)))
        ...
        # deformation_1 = 0.1
        # deformation_2 = 0.01
        # indices0 = np.where((lambda_min_max[:, 0] >= (1-deformation_1)) & (lambda_min_max[:, 1] <= (1+deformation_1)) & (lambda_min_max[:, 0] <= (1-deformation_2)) & (lambda_min_max[:, 1] >= (1+deformation_2)))
        ...
        disps = data_u[indices0[0],:,:].numpy().astype('float64')
        forces = data_f[indices0[0],:,:].numpy().astype('float64')
        ...
        # output_file_name = "%s/%s_%s_%s.mat" % (current_dir, data_name, deformation_1, deformation_2)
        output_file_name = "%s/%s_%s_%s_%s.mat" % (current_dir, data_name, deformation_1, deformation_2, deformation_3)
        savemat(output_file_name, {'coords': coords, "mass": mass, "disps": disps, "forces": forces})
        
        output_file_name = "%s/index_%s_%s_%s.mat" % (current_dir, deformation_1, deformation_2, deformation_3)
        savemat(output_file_name, {'index': indices0[0]})
