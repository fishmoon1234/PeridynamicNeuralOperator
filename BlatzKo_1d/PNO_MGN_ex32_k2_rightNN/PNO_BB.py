...
def main():
    ...
    for index in range(4):
        ...
        # model = E_GCL_GKN(phi_layer, act_fun_xi).to(device)
        model = E_GCL_GKN(phi_2_layer, act_fun_xi).to(device)
        
        # # trained_model_path = '%s/Results/ex32_128_4_ntrain_300_lrs_[0.005]_lr[0.995, 0.998]_gap_1' % (current_dir)
        # trained_model_path = '%s/Results/ex32_256_6_ntrain_300_lrs_[0.001]_lr[0.992, 0.998]_gap_1' % (current_dir)
        # model.load_state_dict(torch.load('%s/model.ckpt' % trained_model_path))
