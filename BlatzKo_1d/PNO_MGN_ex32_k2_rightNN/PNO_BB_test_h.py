...
plt.title(r'learned kernel $k_2(\xi)$, ntrain=%s'% ntrain)
plt.legend()
plt.tight_layout()
# plt.savefig('%s/%s_k_h_%s_%s.png' % (base_dir, ex, h[i], act_xi), format='png')
plt.savefig('%s/%s_kernel_ntrain_%s.png' % (current_dir, ex, ntrain), format='png')
plt.close()
print('---')
...
plt.plot(h, rel_L2_err_k2, 's--', markerfacecolor='none',color='yellowgreen', linewidth=2, label=r"rel $L^2$ error")
# plt.plot(h, h/h[1]*L2_err_k2[1], 'k', label='slope=1', linewidth=2)
plt.plot(h, (h/h[1])**2*L2_err_k2[1], 'k--', label='slope=2', linewidth=2)
...
plt.title(r'error of $k_2(\xi)$, ntrain=%s'% ntrain)
plt.grid(True, which="both", ls="--", color='gray')
plt.legend()
plt.show()
plt.tight_layout()
# name = '%s_error_k2_ntrain_%s_%s_h.png' % (ex, ntrain, act_xi)
name = '%s_error_k2_ntrain_%s_%s.png' % (ex, ntrain, act_xi)
...
# plt.yscale('log')
# plt.title(r'measure $\rho(\xi)$, ntrain=%s'% ntrain)
# plt.grid(True, which="both", ls="--", color='gray')
# plt.legend()
# plt.tight_layout()
