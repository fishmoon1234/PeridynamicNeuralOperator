...
for i in range(N):
    ...
    rel_L2_err_b[i] = np.sqrt(loss[-1,1])
...
ax1.plot(h, rel_L2_err_b, 's--', markerfacecolor='none',color='tomato', linewidth=2)
# plt.plot(h, h/h[1]*L2_err_k2[1], 'k', label='slope=1', linewidth=2)
ax1.plot(h, (h/h[-1])**2*rel_L2_err_b[-1], 'k', label='slope=2', linewidth=2)
...
plt.title(r'Learn $k(\lambda,\xi)$: error of test $b$')          
plt.tight_layout()
name = '%s_error_b_ntrain_%s.png' % (ex, ntrain)
