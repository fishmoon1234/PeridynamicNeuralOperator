...
class mGradNet_C_z(nn.Module):
    ...
    def forward(self, x):
        ...
        for _ in range(self.current_layers):
            ...
            z = coef * (skip + F.softplus(self.alpha_shared).view(1, -1) * self.nonlinearity(z))
...
class E_GCL_GKN(nn.Module):
    ...
    def __init__(self, num_layers, embed_dim, phi_2_layer, act_fun_xi=nn.ReLU(), act_fn=nn.Softplus(), normalize=False, coords_agg='mean', residual=True):

        super().__init__()

        self.act_fun_xi = act_fun_xi
        self.act_fn = act_fn
        # self.phi_MGN = mGradNet_C_z(num_layers, start_layer, 1, embed_dim, self.act_fn )
