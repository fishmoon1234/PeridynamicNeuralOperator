    ...
    for index in range(3, 4):
            ...
            for learning_rate in lrs:
                for scheduler_gamma in gammas:
                    for weight_decay in wds:
                        ...
                        print("-" * 100)
                        print("-" * 100)
                        print(f'>> ntrain: {ntrain}, layer_info: {layer_info}, act_xi: {act_xi}, lr: {learning_rate}, gamma: {scheduler_gamma}, w_d: {weight_decay}')
                        ...
                        f.write(f'{ntrain}, {lrs}, {lr}, {layer_info}, {dx}, {act_xi}: ')
        ...
        ################################################################
            ...
            for batch in test_loader:
                ...
                j += 1
            print(f'>> Error of b: {np.mean(err):.4e}')
            ...
            if if_plot_g == 1:
                ...
                Lambda_rho = torch.linspace(0.8109,3.3456, Nlambda)
                ...
                g_NN = (model.phi_1(Lambdaa_cuda)- model.phi_1(Lambdaa_1_cuda)).reshape(-1,1).cpu().detach()
                
                err = torch.norm(g_NN-g_true)/torch.norm(g_true)
                print(f'>> Error of g: {err:.4e}')
                ...
                plt.plot(Lambdaa, g_NN, color='darkorange', linestyle='--', linewidth=2, label='MLP')
                ...
                plt.savefig('%s/%s_g_MLP.png' % (base_dir, ex), format='png', bbox_inches='tight',dpi=300)
                plt.close()
