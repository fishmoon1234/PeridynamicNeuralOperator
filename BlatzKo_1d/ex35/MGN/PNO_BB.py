    ...
    for index in range(3, 4):
        ...
        ################################################################
            ...
            if if_plot_g == 1:
                ...
                Lambdaa_cuda = Lambdaa.to(device)
                Lambdaa_1_cuda = torch.ones_like(Lambdaa_cuda)
                g_true = g_fun(Lambdaa)
                g_NN = (model.phi_MGN(Lambdaa_cuda)- model.phi_MGN(Lambdaa_1_cuda)).reshape(-1,1).cpu().detach()
                ...
                plt.rcParams.update({'font.size': fontsize}) 
                fig, ax = plt.subplots(figsize = (8,6))
                plt.plot(Lambdaa, g_NN, color='darkorange', linestyle='--', linewidth=2, label='MGN')
                ...
                plt.savefig('%s/%s_g_MGN.png' % (base_dir, ex), format='png', bbox_inches='tight',dpi=300)
                plt.close()
