...
# ICNN model
model_ICNN = E_GCL_GKN_ICNN(phi_1_layer, nn.Softplus()).to(device)
model_ICNN_path = os.path.join(current_dir, '../ICNN/Results/ex35_%s_ntrain_300_lrs_[0.01]_lr_[0.99, 0.998]_gap_1_Softplus_seed_42' % layer_info)
...
for i in range(3,N):
    ...
    # chosed model
    base_dir = 'Results/ex35_%s_ntrain_300_lrs_[0.01]_lr_[0.99, 0.998]_gap_1_%s_seed_42' % (layer_info, act_xi)
    ...
    err_NN_OOD = torch.sqrt(torch.sum((k1_NN-k1_true)**2))/torch.sqrt(torch.sum((k1_true)**2))
    err_MGN_OOD = torch.sqrt(torch.sum((k1_MGN-k1_true)**2))/torch.sqrt(torch.sum((k1_true)**2))
    err_ICNN_OOD = torch.sqrt(torch.sum((k1_ICNN-k1_true)**2))/torch.sqrt(torch.sum((k1_true)**2))
    
    print("NN OOD error:", err_NN_OOD)
    print("MGN OOD error:", err_MGN_OOD)
    print("ICNN OOD error:", err_ICNN_OOD)
...
if if_plot == 1:
    ...
    plt.rcParams.update({
        'font.size': fontsize,
        'font.family': 'serif',
        'axes.linewidth': 1.2,          # uniform width for all axes
        'axes.edgecolor': 'gray', 
        # 'xtick.major.width': 2,
        # 'ytick.major.width': 2,
    ...
    plt.savefig('%s/%s_g_NN_MGN_ICNN_%s_%s_new.png' % (current_dir, ex, layer_info, act_xi), 
    ...
    # Additional plotting functionality can be added here if needed
