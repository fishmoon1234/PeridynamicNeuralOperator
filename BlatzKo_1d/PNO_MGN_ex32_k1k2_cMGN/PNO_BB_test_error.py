...
for i in range(N):
    ...
    rho_xi = torch.abs(k1_fun(lambdaa)*ksi_plus_eta/ksi_plus_eta_norm)
    ...
    for k in range(Nlambda-1):
        ...
        if indices.numel() != 0:
            ...
            weights[k] = torch.sum(torch.abs((g_fun(ksi_norm4rho[index])/ksi_norm4rho[index])*ksi_plus_eta[index]/ksi_plus_eta_norm[index]))/total_nlambda
    ...
    gk_true = k1_fun(Lambda_rho)*2*c/Xi_norm*g_fun(Xi_norm)
...
colors = ['darkorange', 'yellowgreen','deepskyblue', 'mediumslateblue', 'palevioletred', 'gray']
plt.plot(Lambda_plot[-1], k1_true, color='k', linewidth=1.5, label='true') 
for i in range(N):
...
fig, ax = plt.subplots(figsize = (6,5))
colors = ['darkorange', 'yellowgreen','deepskyblue', 'mediumslateblue', 'palevioletred', 'gray']
plt.plot(Xi_plot[-1], k2_true, color='k', linewidth=1.5, label='true') 
for i in range(N):
