...
u_fun = lambda a, b, x: (
    np.sum(
        a*decay*np.sin(feq*np.pi*np.asarray(x).reshape(1, -1)) +
        b*decay*np.cos(feq*np.pi*np.asarray(x).reshape(1, -1)),
        axis=0
    )
    + C * (np.asarray(x).reshape(-1) >= x0).astype(float)
)
...
while n < N_data:
    ...
    # axes[0].plot(xh_all, u)
    # axes[0].set_title("u")
    # axes[0].set_xlabel("x")
    # axes[0].set_ylabel("u(x)")
    # axes[0].tick_params(axis='both', which='major')  # adjustable tick style

    # axes[1].plot(xh_all, bodyforce[n, :])
    # axes[1].set_title("b")
    # axes[1].set_xlabel("x")
    ...
    # plt.tight_layout()
    # plt.savefig(f"{current_dir}/{ex}_data.png", format='png')
    # plt.close(fig)
    # print("---")
...
data_name = "BK_ex38_ndata_%s_Nx_%s_delta_%s_h_%s.mat" % (N_data, Nx, delta, h)
