...
h0 = 2**(-8)
ntrain = 300
# Nlambda = np.array([10, 20, 40, 80, 160], dtype=int)
# Nlambda = np.array([40, 40, 40, 40, 40], dtype=int)
Nlambda = np.array([10, 10, 10, 10, 10], dtype=int)
# Nlambda = np.array([160, 160, 160, 160, 160], dtype=int)
...
for i in range(N):
    ...
    print((torch.min(lambdaa),torch.max(lambdaa)))
    
    lambda_min_all_data, lambda_max_all_data = torch.min(lambdaa), torch.max(lambdaa)
    # lambda_min_all_data, lambda_max_all_data = 0.9, 1.1
    
    Lambda_rho = torch.linspace(lambda_min_all_data, lambda_max_all_data, Nlambda[i])
    ...
    # coe = torch.zeros((ntrain*s, Nlambda))
    ...
    #         coe_mid = (g_fun(ksi_norm4rho[index])/ksi_norm4rho[index])*ksi_plus_eta[index]/ksi_plus_eta_norm[index]
    #         coe[:,k] = torch.sum(coe_mid, dim=-1).reshape(-1,1)
    
    coe = torch.zeros((ntrain, s, Nlambda[i]-1))        
    for ii in range(ntrain):
        for j in range(s):
            for k in range(Nlambda[i]-1):
                index = torch.nonzero((lambdaa[ii,j,:] >= Lambda_rho[k]) & (lambdaa[ii,j,:] <= Lambda_rho[k+1]), as_tuple=False)
                if index.numel() != 0:
                # if indices.numel() != 0:
                    # index = (indices[:,0],indices[:,1],indices[:,2])
                    coe[ii,j,k] = h[i]*torch.sum((g_fun(ksi_norm4rho[ii,j,index])/ksi_norm4rho[ii,j,index])*ksi_plus_eta[ii,j,index]/ksi_plus_eta_norm[ii,j,index])
                       
    coe = coe.reshape(-1, Nlambda[i]-1)   
...
plt.rcParams.update({'font.size': fontsize}) 
fig, ax = plt.subplots(figsize = (6,5))
# plt.plot(h, A_cond, 's--', markerfacecolor='none', color='deepskyblue', linewidth=2, label=r'cond(A)')
plt.plot(h, ATA_cond, 's--', markerfacecolor='none', color='darkorange', linewidth=2, label=r'cond($A^TA$)')
# plt.plot(h, (h/h[-1])**(-4)*cond[-1], 'k', label='slope=1', linewidth=2)
# plt.plot(h, (h/h[1])**(-1)*ATA_cond[1],  'k', label='slope=1', linewidth=2)
# plt.plot(h, (h/h[1])**(-2)*ATAinvAT_L2[1],  'b', label='slope=2', linewidth=2)
...
plt.title(r'Condition number of $A^TA$, nlambda=%s'% Nlambda[-1])
plt.grid(True, which="both", ls="--", color='gray')
# plt.legend()
plt.tight_layout()
name = '%s_cond_nlambda_%s_ntrain_%s.png' % (ex,  Nlambda[-1], ntrain)
