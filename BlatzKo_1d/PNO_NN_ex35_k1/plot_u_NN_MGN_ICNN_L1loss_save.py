...
model_ICNN_path = os.path.join(current_dir, '../PNO_ICNN_ex35_k1/Results/ex35_%s_ntrain_300_lrs_[0.01]_lr_[0.99, 0.998]_gap_1' % layer_info)
model_path_icnn = os.path.join(model_ICNN_path, 'model.ckpt')
model_ICNN.load_state_dict(torch.load(model_path_icnn))
model_ICNN.eval()
...
for i in range(3, N):
    gap = int(h[i]/h0)
    # Load NN model
    # base_dir = 'Results/ex35_20_4_ntrain_300_lrs_[0.01]_lr_[0.995, 0.998]_gap_1_ReLU_L1loss_seed_43'
    ...
    plot_index = [2]  # Only process sample 2 for saving
    ...
    if compute == 1:
        ...
        with torch.no_grad():
            ...
            for j in plot_index:
                ...
                # Save the results for plot_index=2
                if j == 2:
                    # Use the same directory structure as base_dir
                    save_dir = base_dir  # Reuse the base_dir where model is loaded from
                    os.makedirs(save_dir, exist_ok=True)
                    # Create a filename with h value in a readable format
                    h_str = f'h{int(-np.log2(h[i]))}'  # Convert h=2^(-8) to h8
                    save_path = os.path.join(save_dir, f'solved_u_plot_index_{j}_{h_str}.npz')
                    
                    # Save all relevant data for plotting
                    np.savez(save_path,
                             data_X=data_X.cpu().numpy(),
                             u_true=u_true.cpu().numpy(),
                             uh_NN=uh_NN.cpu().numpy(),
                             uh_MGN=uh_MGN.cpu().numpy(),
                             uh_ICNN=uh_ICNN.cpu().numpy(),
                             ...
                             s=s,
                             S=S)
                    print(f"Saved solved u values for plot_index={j}, h={h[i]} ({h_str}) to {save_path}")
                
                plot_u_NN_MGN_ICNN(data_X, u_true, uh_NN, uh_MGN, uh_ICNN, j, current_dir, '%s_%s_%s' % (ex, initial_type, layer_info))
