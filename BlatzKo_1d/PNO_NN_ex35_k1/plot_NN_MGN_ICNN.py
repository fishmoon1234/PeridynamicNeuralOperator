...
for i in range(3,N):
    gap = int(h[i]/h0)
    base_dir = 'Results/%s_%s_ntrain_%s_lrs_%s_lr_%s_gap_%s_%s' % (ex, layer_info, ntrain, lrs, lr, gap, act_xi)
...
if if_plot == 1:
    ...
    plt.savefig('%s/%s_k_NN_MGN_ICNN_%s_%s.png' % (current_dir, ex, layer_info, act_xi), 
