...
model = E_GCL_GKN(phi_1_layer, nn.Sigmoid()).to(device)
...
for i in range(3, N):
    ...
    Lambdaa = torch.linspace(lambda_min_data[i], lambda_max_data[i], 100)
...
ax1.plot(h, rel_L2_err_k1, 's--', markerfacecolor='none',color=color_err, linewidth=2, label=r"rel $L^2$ error")
plt.plot(h, h/h[-1]*rel_L2_err_k1[-1], 'k', label='slope=1', linewidth=2)
ax1.plot(h, (h/h[-1])**2*rel_L2_err_k1[-1], 'k--', label='slope=2', linewidth=2)
ax1.set_xscale('log', base=2)
ax1.set_yscale('log')
ax1.set_xlabel('Mesh size')
ax1.set_ylabel(r'rel $L^2$ error of $g(\lambda)$', color=color_err)
...
plt.tight_layout()
name = '%s_error_g_%s_%s_new.png' % (ex, lambda_min, layer_info)
...
plt.legend()
plt.tight_layout()
name = '%s_rho_lambda_ntrain_%s_new.png' % (ex, ntrain)
plt.savefig (os.path.join (current_dir, name))
plt.close()
